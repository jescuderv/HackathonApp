...
public class PreferencesFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mRecyclerView = (RecyclerView) view.findViewById(R.id.recycler_view_event_assist);
    ...
    private void listarElementos(){
        ArrayList<Evento> listaFavoritos = new ArrayList<>();

        mRealm.beginTransaction();
        RealmResults<User> user = mRealm.where(User.class).findAll();
        for (User user1 : user){
            if (user1.getEventListFavorites() != null) {
                for (Evento evento : user1.getEventListFavorites())
